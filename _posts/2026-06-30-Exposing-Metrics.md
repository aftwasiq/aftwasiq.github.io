...
We don't return the counter variable or anything. Scrutiny will instrument its value through the varmap generated by the SFD or as a runtime published variable (RPV) if initialized otherwise.
...
*Later edit: better to probably use `volatile`*
...
My initial approach involved using rtems_interrupt_catch to "catch" whenever an interrupt fired...
...
for (uint32_t i = 0; i < LEON3_VECTOR_COUNT; i++) { /* or STM32F4_VECTOR_COUNT */
    ...
    // directive_failed( status, "interrupt counter" );
}
```
